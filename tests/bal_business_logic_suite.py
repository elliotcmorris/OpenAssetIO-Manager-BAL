...
class Test_resolve_trait_property_expansion(LibraryOverrideTestCase):
    ...
    @unittest.skipIf(
        sys.platform == "win32", reason="Constructing path expectations requires appropriate OS"
    )
    def test_bal_library_dir_path_from_url_compatible(self):
        ...
        path = converter.pathFromUrl(substituted_url, PathType.kPOSIX)
        self.assertEqual(path, expected_path)

    @unittest.skipIf(
        sys.platform != "win32", reason="Constructing path expectations requires appropriate OS"
    )
    def test_bal_library_dir_path_from_url_compatible_windows(self):
        expected_path = os.path.join(os.path.dirname(__file__), "aboveFile.txt")
        converter = FileUrlPathConverter()
        data = self.__resolve_to_dict()
        substituted_url = data["relative_to_bal_library_dir_url_windows"]
        path = converter.pathFromUrl(substituted_url, PathType.kWindows)
        self.assertEqual(path, expected_path)
